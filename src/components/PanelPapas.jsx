...
const PanelPapas = () => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        ...
        const respuestaMaterias = await axios.get('http://localhost:5000/api/materias', config);
        setMaterias(respuestaMaterias.data);
        if (respuestaMaterias.data.length > 0) setMateriaSeleccionada(respuestaMaterias.data[0].id);

        const respuestaMision = await axios.get('http://localhost:5000/api/progreso', config);
  ...
  const enviar = async (puntos) => {
    ...
    try {
      await axios.put('http://localhost:5000/api/puntaje', {
