...
const RegistroPiloto = () => {
  ...
  const cargarPilotos = async () => {
    try {
      const res = await axios.get('https://mision-espacial-backend.onrender.com/api/pilotos');
  ...
  const registrar = async () => {
    ...
    setCargando(true);
    try {
      await axios.post('https://mision-espacial-backend.onrender.com/api/registrar-piloto', { kid_name: nombre, kid_photo_url: foto });
      setNombre(''); 
      setFoto(''); 
      await cargarPilotos();
  ...
  const confirmarAccion = async () => {
    ...
    try {
      if (type === 'borrar') {
        await axios.delete(`https://mision-espacial-backend.onrender.com/api/pilotos/${pilot.id}`);
        lanzarAviso(`🗑️ PILOTO ${pilot.kid_name.toUpperCase()} ELIMINADO`);
      } else if (type === 'editar') {
        await axios.put(`https://mision-espacial-backend.onrender.com/api/pilotos/${pilot.id}`, { kid_name: nuevoNombre });
  ...
  const seleccionar = async (id) => {
    try {
      await axios.post('https://mision-espacial-backend.onrender.com/api/seleccionar-piloto', { id });
      navigate('/explorar'); 
